...
describe('fetchRevenue', () => {
  it('returns revenue in cents', async () => {
    ...
    await expect(fetchRevenue(1, '2022-01-05', '2022-01-06')).resolves.toBe(737261);
  ...
  it('returns null when revenue is not yet available', async () => {
    ...
    await expect(fetchRevenue(1, '2022-01-05', '2022-01-06')).resolves.toBeNull();
  });

  it('throws on other errors', async () => {
    mockFetch(400, 'Revenue unavailable for future day');
    await expect(fetchRevenue(1, '2022-01-06', '2022-01-06')).rejects.toMatchObject({
      status: 400,
    });
  });
});
