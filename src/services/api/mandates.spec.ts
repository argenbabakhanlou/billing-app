...
describe('chargeMandate', () => {
  it('posts the amount as a two-decimal string and returns true', async () => {
    ...
    await expect(chargeMandate(2, 442357, '2022-01-06')).resolves.toBe(true);
  ...
  it('returns false when the charge is rejected', async () => {
    ...
    await expect(chargeMandate(2, 100, '2022-01-06')).resolves.toBe(false);
  });

  it('throws on other errors', async () => {
    mockFetch(400, 'bad');
    await expect(chargeMandate(2, 100, '2022-01-06')).rejects.toMatchObject({ status: 400 });
  });
});
